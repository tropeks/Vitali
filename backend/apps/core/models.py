...
class AuditLog(models.Model):
    ...
    Order 020 — partitioned + retention: the table is RANGE-partitioned by
    month on ``created_at`` and each month is LIST-partitioned by
    ``schema_name`` (migration 0043), with a DEFAULT partition at both levels
    ...
    deliberate trade (confirmed there is no FK/O2O/M2M anywhere in the
    backend pointing at AuditLog, and nothing looks it up by bare ``pk``/
    ``id``): tenant isolation at the DDL level was judged worth more than a
    DB-enforced globally-unique id. ``id`` itself keeps incrementing via the
    SAME Postgres identity column it always had (``GENERATED BY DEFAULT AS
    IDENTITY``) — it is just no longer, on its own, the primary key.

    ``id`` is declared as a plain (non-auto) field with a placeholder
    ``db_default`` rather than ``BigAutoField``: Django requires an AutoField
    to be the model's sole primary key (fields.E100), which a composite pk
    model cannot satisfy. ``db_default`` only needs to make
    ``has_db_default()`` True so Django emits SQL ``DEFAULT`` (not ``NULL``)
    on INSERT — Postgres' own identity default (installed by raw SQL in
    migration 0043, via ``SeparateDatabaseAndState`` — Django never generates
    real DDL for this column) then assigns the value, and Django reads it
    back via RETURNING. The literal ``0`` is never actually used as a value.
    """
    ...
    # Partition key (created_at, schema_name) must be part of the primary
    # key — Postgres requirement for partitioned tables. Must be named "pk"
    # (Django's fields.E013). See the class docstring for the full rationale.
    # django-stubs 5.1 predates Django 5.2's CompositePrimaryKey (added this
    # release) — attr-defined is a stub-lag false positive, not a real typing
    # gap; remove the ignore once django-stubs catches up.
    pk = models.CompositePrimaryKey(  # type: ignore[attr-defined]
        "id", "created_at", "schema_name"
    )
    ...
    def save(self, *args, **kwargs):
        # Enforce append-only: block updates. NOTE: this used to check
        # `if self.pk:`, which worked when pk was the bare `id` AutoField
        # (None until the first save). With a CompositePrimaryKey, `self.pk`
        # is always a same-length tuple — e.g. (None, None, "") for a
        # brand-new instance — which is truthy regardless of content, so
        # that check would now reject every INSERT. `_state.adding` is
        # Django's own "has this instance ever been saved" flag (see
        # Model.save_base) and is correct independent of pk shape.
...
from .adt_catalog_models import *  # noqa: E402,F401,F403
from .blood_catalog_models import *  # noqa: E402,F401,F403
