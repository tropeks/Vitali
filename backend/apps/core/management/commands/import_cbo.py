"""
Management command: import_cbo  (core — M2-S1-T1)
=================================================
Imports a CBO (Classificação Brasileira de Ocupações) occupation table into
``core.CBOCode``, keyed on the CBO code. Uses the E1-T1
...
(provenance = MTE/DATASUS).

Usage:
    python manage.py import_cbo --source /path/to/cbo.csv
    python manage.py import_cbo --source cbo.csv --cbo-version 2024 --dry-run

Expected CSV (semicolon-delimited, UTF-8; lines starting with '#' are comments):
    CODIGO;TITULO;FAMILIA

Only CODIGO and TITULO are required; FAMILIA is optional and left at its inert
default when absent. No value is fabricated here — the importer copies only what
the CBO source row provides. Rows with a blank code / title are reported by
physical line number and abort the run (fail-loud), so nothing is half-imported.
"""
...
# Header aliases (CBO exports vary) → canonical row keys.
_COLUMN_ALIASES = {
    "code": ("CODIGO", "codigo", "Código", "COD", "CBO", "code"),
    "display": ("TITULO", "titulo", "Título", "OCUPACAO", "ocupacao", "display"),
    "family": ("FAMILIA", "familia", "Família", "FAMILIA_CBO", "family"),
}


class CBOImporter(CatalogImporter):
    """CatalogImporter bound to CBOCode, keyed on (system, codigo, version)."""
...
class Command(BaseCommand):
    help = "Import a CBO occupation catalog into core.CBOCode"

    def add_arguments(self, parser):
        parser.add_argument(
            "--source", required=True, help="Path to the CBO CSV (semicolon-delimited, UTF-8)"
        )
        parser.add_argument("--delimiter", default=";", help="CSV column delimiter (default: ;)")
        # dest is 'cbo_version' — NOT 'version' (would collide with BaseCommand's
    ...
    def handle(self, *args, **options):
        ...
        version = options["cbo_version"]
        dry_run = options["dry_run"]

        self.stdout.write(f"Importing CBO catalog from {source_path} (dry_run={dry_run}) …")
        rows = self._parse(source_path, delimiter)

        importer = CBOImporter(
            version=version,
