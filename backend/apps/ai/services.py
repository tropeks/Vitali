...
def check_monthly_ceiling(schema_name: str) -> bool:
    ...
    try:
        ...
        logger.warning(
            "Redis unavailable for monthly ceiling check (tenant=%s) — failing open", schema_name
        )
        return False
...
def _get_prompt_template(name: str = "tuss_suggest") -> AIPromptTemplate | None:
    try:
        return (
            AIPromptTemplate.objects.filter(name=name, is_active=True).order_by("-version").first()
        )
    except Exception:
        return None
