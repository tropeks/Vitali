...
class TUSSSuggestViewTest(TenantTestCase):
    def setUp(self):
        ...
        self._override = override_settings(
            FEATURE_AI_TUSS=True, ANTHROPIC_API_KEY="test-key", AI_RATE_LIMIT_PER_HOUR=1000
        )
    ...
    def test_feature_flag_off_returns_degraded(self):
        ...
        with override_settings(FEATURE_AI_TUSS=False):
            resp = self.client.post(
                "/api/v1/ai/tuss-suggest/", {"description": "consulta"}, format="json"
            )
    ...
    def test_returns_suggestions_on_success(self):
        ...
        with (
            ...
            resp = self.client.post(
                "/api/v1/ai/tuss-suggest/",
                {"description": "consulta cardiologia", "guide_type": "consulta"},
                format="json",
            )
    ...
    def test_cache_hit_returns_cached_true(self):
        ...
        with (
            ...
            self.client.post(
                "/api/v1/ai/tuss-suggest/",
                {"description": "consulta", "guide_type": "consulta"},
                format="json",
            )
...
class TUSSSuggestFeedbackViewTest(TenantTestCase):
    ...
    def test_marks_suggestion_accepted(self):
        ...
        resp = self.client.post(
            "/api/v1/ai/tuss-suggest/feedback/",
            {
...
class AIUsageViewTest(TenantTestCase):
    def setUp(self):
        ...
        AIUsageLog.objects.create(
            event_type="llm_call", tokens_in=50, tokens_out=20, latency_ms=300
        )
