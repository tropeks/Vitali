...
class ClaudeGateway(LLMGateway):
    ...
    def __init__(self, api_key: str | None = None, model: str = MODEL_HAIKU, timeout: int | None = None):
        self.model = model
    ...
    def complete(self, system: str, user: str, max_tokens: int = 512) -> tuple[str, int, int]:
        try:
            ...
            raise LLMGatewayError("anthropic SDK not installed. Add anthropic>=0.40 to requirements.")
        ...
        try:
            ...
            logger.debug("Claude call: %dms, in=%d out=%d", elapsed, message.usage.input_tokens, message.usage.output_tokens)
