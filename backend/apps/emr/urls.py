...
from .views_safety import PrescriptionItemSafetyCheckView, AcknowledgeSafetyAlertView
from .views_cid10 import CID10SuggestView, CID10AcceptView
from .views_pdf import PrescriptionPDFView
from .views_waitlist import WaitlistViewSet, WaitlistDetailView
...
urlpatterns = router.urls + [
    ...
    path('emr/setup/professional/', WizardProfessionalSetupView.as_view(), name='wizard-professional-setup'),
    path('emr/setup/status/', WizardStatusView.as_view(), name='wizard-status'),
    # S-063: AI Prescription Safety
    path('prescription-items/<uuid:item_id>/safety-check/', PrescriptionItemSafetyCheckView.as_view(), name='prescription-safety-check'),
    path('safety-alerts/<uuid:alert_id>/acknowledge/', AcknowledgeSafetyAlertView.as_view(), name='safety-alert-acknowledge'),
    # S-064: AI CID-10 Suggester
    path('encounters/<uuid:encounter_id>/cid10-suggest/', CID10SuggestView.as_view(), name='cid10-suggest'),
    path('encounters/<uuid:encounter_id>/cid10-accept/', CID10AcceptView.as_view(), name='cid10-accept'),
    # S-065: Prescription PDF
    path('prescriptions/<uuid:prescription_id>/pdf/', PrescriptionPDFView.as_view(), name='prescription-pdf'),
    ...
    path('encounters/<uuid:encounter_id>/scribe/start/', ScribeStartView.as_view(), name='scribe-start'),
    path('encounters/<uuid:encounter_id>/scribe/status/', ScribeStatusView.as_view(), name='scribe-status'),
    # S-073: Whisper API Transcription Fallback
    path('encounters/<uuid:encounter_id>/scribe/transcribe/', ScribeTranscribeView.as_view(), name='scribe-transcribe'),
]
