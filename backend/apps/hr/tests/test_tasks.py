...
Covers decision 1B (fail-open cascade):
  1. Success path → AuditLog `whatsapp_channel_created` written.
  ...
  6. Max retries exceeded → AuditLog `whatsapp_setup_failed` written.
...
class TestSetupStaffWhatsappChannel(TenantTestCase):
    ...
    def test_max_retries_writes_failed_audit_log(self):
        ...
        AuditLog `whatsapp_setup_failed` must be written with reason and error.
