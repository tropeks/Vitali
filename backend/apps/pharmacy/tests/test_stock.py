...
class TestStockNegativeGuard(TenantTestCase):
    def test_adjustment_negative_stock_raises(self):
        ...
        with self.assertRaises(ValueError) as ctx:
            StockMovement(
                stock_item=item, movement_type="adjustment", quantity=Decimal("-10")
            ).save()
...
class TestStockEntryValidation(TenantTestCase):
    def test_expiry_date_past_raises_via_serializer(self):
        ...
        item = StockItem.objects.create(drug=drug, lot_number="EXP001", expiry_date=yesterday)
        s = StockMovementSerializer(
            data={
                "stock_item": str(item.id),
                "movement_type": "entry",
...
class TestStockAvailabilityEndpoint(TenantTestCase):
    def setUp(self):
        ...
        self.role = Role.objects.create(
            name="farmaceutico", permissions=DEFAULT_ROLES["farmaceutico"]
        )
    ...
    def test_stock_availability_authenticated_returns_lots(self):
        ...
        item = StockItem.objects.create(
            drug=drug, lot_number="AV1", expiry_date=future, quantity=Decimal("0")
        )
