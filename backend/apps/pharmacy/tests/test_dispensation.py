...
from apps.pharmacy.models import Dispensation, DispensationLot, Drug, StockItem, StockMovement
...
class TestFEFOUnit(TenantTestCase):
    def test_fefo_selects_earliest_non_expired_lot_first(self):
        ...
        lots = StockItem.objects.filter(drug=drug, quantity__gt=0, expiry_date__gte=today).order_by(
            "expiry_date"
        )
    ...
    def test_dispensation_total_quantity_equals_sum_of_lots(self):
        ...
        patient = Patient.objects.create(
            full_name="P", birth_date="1990-01-01", gender="M", cpf="00000000000"
        )
        prof = Professional.objects.create(
            user=user, council_type="CRM", council_number="99", council_state="SP"
        )
        ...
        rx_item = PrescriptionItem.objects.create(
            prescription=rx, drug=drug, quantity=Decimal("12")
        )
...
class TestFEFOIntegration(TenantTestCase):
    def setUp(self):
        ...
        self.encounter = Encounter.objects.create(
            patient=self.patient, professional=self.prescriber
        )
    ...
    def test_fefo_spans_multiple_lots(self):
        ...
        rx, rx_item = _make_prescription(
            self.patient, self.prescriber, self.encounter, drug, qty=Decimal("10")
        )
    ...
    def test_fefo_no_negative_stock_under_lock(self):
        ...
        rx, rx_item = _make_prescription(
            self.patient, self.prescriber, self.encounter, drug, qty=Decimal("99")
        )
        resp = self._client(self.farmaceutico).post(
            "/api/v1/pharmacy/dispense/",
            {
    ...
    def test_controlled_substance_role_gate(self):
        ...
        resp = self._client(self.enfermeiro).post(
            "/api/v1/pharmacy/dispense/",
            {
                "prescription_item_id": str(rx_item.id),
                "quantity": "2",
    ...
    def test_dispense_fills_lot_exactly(self):
        ...
        self._client(self.farmaceutico).post(
            "/api/v1/pharmacy/dispense/",
            {
                "prescription_item_id": str(rx_item.id),
                "quantity": "10",
    ...
    def test_dispensation_lot_atomicity(self):
        ...
        _make_lot(drug, Decimal("5"), days_until_expiry=30)
        rx, rx_item = _make_prescription(
            self.patient, self.prescriber, self.encounter, drug, qty=Decimal("20")
        )
    ...
    def test_over_dispense_beyond_prescribed_quantity_rejected(self):
        ...
        rx, rx_item = _make_prescription(
            self.patient, self.prescriber, self.encounter, drug, qty=Decimal("5")
        )
        resp = self._client(self.farmaceutico).post(
            "/api/v1/pharmacy/dispense/",
            {
    ...
    def test_stock_availability_requires_pharmacy_read_permission(self):
        ...
        role_recep = Role.objects.create(
            name="recep_av", permissions=DEFAULT_ROLES["recepcionista"]
        )
        recep_user = User.objects.create_user(
            email="recep_av@t.com", password="pw", role=role_recep
        )
        from rest_framework.test import APIClient
