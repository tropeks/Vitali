    ...
    <div ref={containerRef} className="relative">
      <div className="relative">
        <input
          ...
          disabled={disabled}
          className="w-full border border-slate-200 rounded-lg px-3 py-2 pr-8 text-sm focus:ring-2 focus:ring-blue-500 outline-none disabled:bg-neu-panel"
        />
        {(value || query) && !disabled && (
          <button
            type="button"
            onClick={clear}
            className="absolute right-2 top-1/2 -translate-y-1/2 text-slate-400 hover:text-neu-inkSoft text-lg leading-none"
            tabIndex={-1}
          >
            ×
          </button>
      ...
      {open && results.length > 0 && (
        <ul className="absolute z-50 mt-1 w-full bg-neu-panel border border-slate-200 rounded-lg shadow-neu-panel max-h-60 overflow-y-auto text-sm">
          {results.map(opt => (
            <li key={opt.id}>
              <button
                type="button"
              ...
              >
                ...
                <span className="text-neu-inkSoft">{opt.description.slice(0, 80)}</span>
              </button>
            </li>
          ))}
        </ul>
