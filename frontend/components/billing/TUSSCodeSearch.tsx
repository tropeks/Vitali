    ...
    <div ref={containerRef} className="relative">
      <div className="relative">
        <input
          ...
          disabled={disabled}
          className="w-full border border-slate-200 rounded-lg px-3 py-2 pr-8 text-sm focus:ring-2 focus:ring-blue-500 outline-none disabled:bg-[#F4F7FA]"
        />
        {(value || query) && !disabled && (
          <button
            type="button"
            onClick={clear}
            className="absolute right-2 top-1/2 -translate-y-1/2 text-slate-400 hover:text-[#57606A] text-lg leading-none"
            tabIndex={-1}
          >
            ×
          </button>
      ...
      {open && results.length > 0 && (
        <ul className="absolute z-50 mt-1 w-full bg-[#F4F7FA] border border-slate-200 rounded-lg shadow-[inset_0_1px_2px_rgba(255,255,255,0.8),_0_2px_8px_rgba(0,0,0,0.03)] max-h-60 overflow-y-auto text-sm">
          {results.map(opt => (
            <li key={opt.id}>
              <button
                type="button"
              ...
              >
                ...
                <span className="text-[#57606A]">{opt.description.slice(0, 80)}</span>
              </button>
            </li>
          ))}
        </ul>
