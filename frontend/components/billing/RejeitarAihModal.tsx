...
/**
 * Rejeitar/glosar AIH (sus.write) — POST /api/v1/billing/aih-autorizacoes/
 * {id}/rejeitar/ {motivo}. Situação → rejeitada. Motivo obrigatório (validado no
 * cliente e no servidor; 409 vira mensagem amigável).
 */
export default function RejeitarAihModal({ aihId, numeroAtual, onClose, onRejected }: Props) {
  ...
  async function submit() {
    ...
    setError(null)
    try {
      await apiFetch(`/api/v1/billing/aih-autorizacoes/${aihId}/rejeitar/`, {
        method: 'POST',
      ...
      setError('Não foi possível rejeitar a AIH. Tente novamente.')
    } finally {
      setSubmitting(false)
    }
  }
