...
export default function GlosaRiskBadge({
  ...
  if (state.kind === 'loading') {
    return (
      <div className="mt-1 flex items-center gap-1">
        <div className="h-5 w-20 bg-neu-app rounded-full animate-pulse" />
        <div className="h-5 w-32 bg-neu-app rounded-full animate-pulse opacity-60" />
      </div>
    );
  }
  ...
  if (state.kind === 'degraded') {
    return (
      <div className="mt-1">
        <span className="inline-flex items-center px-2 py-0.5 text-xs border border-slate-200 bg-neu-panel text-slate-400 rounded-full">
    ...
    <div className="mt-1 relative inline-block" ref={tooltipRef}>
      ...
      {tooltipVisible && data.risk_reason && (
        <div
          id={tooltipId}
          role="tooltip"
          className="absolute z-50 left-0 top-6 w-64 bg-slate-900 text-white text-xs rounded-lg px-3 py-2 shadow-neu-panel"
        >
