...
/**
 * Reconciliar APAC (sus.write) — informa o número oficial de 13 dígitos do
 * gestor via POST /api/v1/billing/apac-autorizacoes/{id}/reconciliar/
 * {numero_oficial, data_autorizacao?}. Situação → autorizada. Número inválido/
 * duplicado ou APAC já autorizada → 409 (mensagem amigável).
 */
export default function ReconciliarApacModal({ apacId, numeroAtual, onClose, onReconciled }: Props) {
  ...
  async function submit() {
    ...
    try {
      await apiFetch(`/api/v1/billing/apac-autorizacoes/${apacId}/reconciliar/`, {
        method: 'POST',
      ...
      if (err instanceof ApiError && err.status === 409) {
        setError('Número inválido/duplicado ou APAC já autorizada.')
        return
      }
      setError('Não foi possível reconciliar a APAC. Tente novamente.')
    } finally {
      setSubmitting(false)
    }
  }
