...
export default function MaterialDetailPage() {
  ...
  useEffect(() => {
    fetch(`/api/v1/pharmacy/materials/${id}/`, { headers: {} })
      .then(r => r.json())
    ...
    <div className="space-y-6 max-w-3xl">
      <div className="flex items-start justify-between">
        <div>
          ...
          <h1 className="text-2xl font-semibold text-neu-ink">{material.name}</h1>
          {!material.is_active && (
            <span className="mt-1 inline-block px-2 py-0.5 text-xs font-medium rounded bg-neu-app text-neu-inkMuted">Inativo</span>
          )}
        </div>
          ...
          <div className="grid grid-cols-2 gap-6">
            {[
              ...
              <div key={label}>
                ...
                <p className="text-sm text-neu-ink">{value || '—'}</p>
              </div>
            ))}
            <div>
              <p className="text-xs font-medium text-neu-inkMuted mb-0.5">Última atualização</p>
              <p className="text-sm text-neu-inkMuted">{new Date(material.updated_at).toLocaleDateString('pt-BR')}</p>
            </div>
          </div>
        ) : (
