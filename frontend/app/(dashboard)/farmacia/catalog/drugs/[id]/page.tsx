...
export default function DrugDetailPage() {
  ...
  const field = (label: string, value: string | undefined, mono = false) => (
    <div key={label}>
      <p className="text-xs font-medium text-neu-inkMuted mb-0.5">{label}</p>
      <p className={`text-sm text-neu-ink ${mono ? 'font-mono' : ''}`}>{value || '—'}</p>
    </div>
  )
    ...
    <div className="space-y-6 max-w-3xl">
      <div className="flex items-start justify-between">
        <div>
          ...
          <h1 className="text-2xl font-semibold text-neu-ink">{drug.name}</h1>
          <div className="flex items-center gap-2 mt-1">
            {drug.controlled_class !== 'none' && (
              <span className={`px-2 py-0.5 text-xs font-mono font-semibold rounded ${CONTROLLED_BADGE[drug.controlled_class] || 'bg-neu-app text-neu-inkSoft'}`}>
                {drug.controlled_class_display}
              </span>
            )}
            {!drug.is_active && (
              <span className="px-2 py-0.5 text-xs font-medium rounded bg-neu-app text-neu-inkMuted">Inativo</span>
            )}
          </div>
        </div>
          ...
          {!editing ? (
            ...
            <>
              <button
                onClick={handleSave}
                disabled={saving}
                className="px-4 py-2 text-sm font-medium text-white bg-gradient-to-b from-neu-brand to-neu-brandDeep border-t border-neu-brandEdge shadow-neu-btn-primary rounded-lg hover:shadow-neu-btn-primary-hover disabled:opacity-50"
              >
      ...
      <div className="bg-neu-panel border border-slate-200 rounded-lg p-4">
        {!editing ? (
          <div className="grid grid-cols-2 gap-6">
            ...
            <div>
              <p className="text-xs font-medium text-neu-inkMuted mb-0.5">Última atualização</p>
              <p className="text-sm text-neu-inkMuted">{new Date(drug.updated_at).toLocaleDateString('pt-BR')}</p>
            </div>
          </div>
        ) : (
          <div className="grid grid-cols-2 gap-4">
            {[
              ...
              <div key={key}>
                <label className="block text-xs font-medium text-neu-inkSoft mb-1">{label}</label>
                <input
