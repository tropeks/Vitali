...
const formatMoney = (value?: string) => { const [integer = '0', fraction = '00'] = String(value ?? '0').replace(',', '.').split('.'); return `R$ ${integer.replace(/\B(?=(\d{3})+(?!\d))/g, '.')},${fraction.padEnd(2, '0').slice(0, 2)}` }
...
export default function NFeEntriesPage() {
  ...
  async function reject(invoice: Invoice) {
    setWorking(true); setMessage(null)
    try { const updated = await apiFetch<Invoice>(`/api/v1/pharmacy/nfe-receipts/${invoice.id}/reject/`, { method: 'POST', body: JSON.stringify({ reason: 'Devolução solicitada na conferência' }) }); setItems(current => current.map(item => item.id === invoice.id ? updated : item)); setSelected(updated); setMessage('Devolução registrada e NF-e rejeitada.') }
    catch (e) { setMessage(apiErrorMessage(e, 'Não foi possível registrar a devolução.')) }
    finally { setWorking(false) }
  }
