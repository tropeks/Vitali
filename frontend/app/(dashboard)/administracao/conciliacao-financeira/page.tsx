...
const statusLabel: Record<string, string> = { pending: 'Pendente', matched: 'Conciliado', disputed: 'Divergência', rejected: 'Rejeitado' }
...
export default function FinancialReconciliationPage() {
  ...
  const kpis = useMemo(() => ({ pending: txs.filter(t => t.status === 'pending' || !t.status).length, suggested: txs.filter(t => !!t.suggested_receivable).length, disputed: txs.filter(t => t.status === 'disputed').length }), [txs])
