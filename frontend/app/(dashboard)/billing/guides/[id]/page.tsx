  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3 flex-wrap">
        ...
        {guide?.status && (
          <span className={`inline-flex px-3 py-1 rounded-full text-sm font-medium ${STATUS_BADGE[guide.status] ?? 'bg-slate-100 text-slate-600'}`}>
      ...
      <div className="bg-white rounded-lg border border-slate-200 p-6">
        <h2 className="font-semibold text-slate-900 mb-4">Dados da Guia</h2>
      ...
      {guide?.items && guide.items.length > 0 && (
        <div className="bg-white rounded-lg border border-slate-200 overflow-hidden">
          <div className="px-5 py-4 border-b border-slate-100">
            <h2 className="font-semibold text-slate-900">Procedimentos</h2>
          </div>
            ...
            <tbody className="divide-y divide-slate-50">
              {guide.items.map((item: any, idx: number) => (
                <tr key={item.id ?? idx}>
                  ...
                  <td className="px-4 py-3 text-slate-700">{fmtCurrency(item.unit_value)}</td>
                  <td className="px-4 py-3 text-slate-900 font-medium">{fmtCurrency(item.total_value ?? (item.quantity * item.unit_value))}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          {guide?.glosas && guide.glosas.length > 0 ? (
            <ul className="space-y-2">
              {guide.glosas.map((g: any, i: number) => (
                <li key={g.id ?? i} className="text-sm text-slate-700 border border-slate-100 rounded-lg p-3">
