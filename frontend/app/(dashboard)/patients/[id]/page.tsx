...
import { ArrowLeft, CalendarPlus, FileText, Receipt, ShieldAlert } from 'lucide-react'
...
export default function PatientDetailPage() {
  ...
  const activeAllergies = patient.allergies?.filter((a: any) => a.status === 'active') ?? []
  const lifeThreateningAllergies = activeAllergies.filter((a: any) => a.severity === 'life_threatening')
  const activeConditions = patient.medical_history?.filter((h: any) => h.status === 'active') ?? []
      ...
      <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="flex items-start gap-4">
            ...
            <div className="w-12 h-12 rounded-lg bg-blue-100 flex items-center justify-center text-blue-600 font-semibold text-lg">
              {patient.full_name[0]}
            </div>
            <div className="min-w-0">
              ...
              <div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-gray-500">
                ...
                {patient.blood_type && <><span className="text-slate-300">·</span><span className="font-medium text-red-600">{patient.blood_type}</span></>}
              </div>
            </div>
          </div>
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-3 lg:min-w-[480px]">
            <button
        ...
        {lifeThreateningAllergies.length > 0 && (
          <div className="mt-4 flex items-start gap-2 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm font-semibold text-red-700">
            ...
            Alergia com risco de vida: {lifeThreateningAllergies.map((a: any) => a.substance).join(', ')}
          </div>
        )}

        <div className="mt-4 grid grid-cols-2 gap-3 lg:grid-cols-4">
          <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
            <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Alergias ativas</p>
            <p className="mt-1 text-lg font-bold text-slate-900">{activeAllergies.length}</p>
          </div>
          <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
            <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Condições ativas</p>
            <p className="mt-1 text-lg font-bold text-slate-900">{activeConditions.length}</p>
          </div>
          <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
            <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Convênios</p>
            <p className="mt-1 text-lg font-bold text-slate-900">{patient.insurance_cards?.length ?? '—'}</p>
          </div>
          <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
            <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Contato</p>
            <p className="mt-1 truncate text-sm font-semibold text-slate-900">{patient.phone || patient.whatsapp || 'Não informado'}</p>
          </div>
        </div>
      </div>
