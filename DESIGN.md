...
- **Open encounter tabs:** Clinical workspaces persist recently opened encounters in a horizontal tab rail. Each tab shows patient name, MRN, and encounter status with a close action, so physicians can switch patients without returning to the encounter list.
- **CPOE order grid:** CPOE renders as a dense order surface: status strip, quick clinical presets, inline medication search, order rows for quantity/unit/posology/safety/status, and explicit error states. It should feel like a working hospital order-entry screen, not a generic note card.
